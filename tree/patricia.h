...
struct Nodo{
    ...
    T value;
    char *key;
    unordered_map<char, Edge<T>>* children;


    Nodo(){
        this->isLeaf = false;
        this->children = new unordered_map<char, Edge<T>>();
    }

    Nodo(const string &key, T value){
        this->isLeaf = true;
        this->value = value;
...
public:
    ...
    bool insert(const string &key, T value){
    ...
    vector<string> *listByPrefix(const string &key){
        ...
        while (!q.empty()) {
            ...
            q.pop();
            if (!nodo->isLeaf) {
                typename unordered_map<char, Edge<T>>::iterator it =
    ...
    string largerPrefix(const string &key){
        queue<Nodo<T>*> q;
        q.push(this->root);
        int position = 0;
        while (!q.empty()) {
    ...
    struct iterator{
        ...
        stack<Nodo<T>*> way;
        stack<typename unordered_map<char,Edge<T>>::iterator> child;
        ...
        string operator*() const{
            return string(this->way.top()->key);
        }
...
private:
    ...
    bool insert(Nodo<T>*& nodo, const string &key, T value, int position){
        if (!nodo->isLeaf){
            ...
            if (it != nodo->children->end()) {
                ...
                while (true) {
                    if (position == int(key.size())) return false;
                    if (position == edge->ip)
    ...
    Nodo<T>* get(Nodo<T>*& nodo, const string &key, int position, stack<Nodo<T>*> &way){
        if (nodo->isLeaf){
            if (int(key.size()) == position){
                return nodo;
            }
            ...
            if (it != nodo->children->end()){
                ...
                if (int(key.size()) < edge->ip) return nullptr;
    ...
    void destroy(Nodo<T> *nodo){
        if (!nodo->isLeaf) {
            for (typename unordered_map<char, Edge<T>>::iterator it =
                 nodo->children->begin();
    ...
    void elements(Nodo<T>* nodo, vector<T>* res){
        if(nodo->isLeaf){
    ...
    vector<string> * keys(Nodo<T> *nodo){
